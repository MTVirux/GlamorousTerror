...
public sealed class GlamourerChangelog : IUiService
{
    ...
    {
        _config   = config;
        Changelog = new Changelog("Glamourer Changelog (synced with upstream Glamourer 1.6.1.7)", ConfigData, Save);
    ...
    private static void AddGlamorousTerrorFeatures(Changelog log)
        => log.NextVersion("Glamorous Terror Features"u8)
            .RegisterHighlight("FIX: BLM/WHM cross job weapon preview getting stuck"u8)
            ...
            .RegisterEntry("Try On from context menus — right-click an equippable item in your inventory, in shops and exchange menus, the market board, or item links to instantly apply it to your character."u8)
            .RegisterEntry("Gear name language selection — override the display language for equipment item names."u8)
            .RegisterEntry("Cross-language equipment search — search for items in any language regardless of display settings."u8)
