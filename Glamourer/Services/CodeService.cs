...
public sealed class CodeService : IService
{
    ...
    public void Toggle(CodeFlag flag, bool enable)
    {
        if (enable)
        {
            ...
            _enabled = (_enabled | flag) & badFlags;
        }
        else
        {
            _enabled &= ~flag;
        ...
        _config.EnabledCheats = _enabled;
        _config.Save();
    }
    ...
    private static CodeFlag GetMutuallyExclusive(CodeFlag flag)
        => flag switch
        {
            ...
            CodeFlag.Face         => (FullCodes | RaceCodes | SizeCodes | GearCodes | DyeCodes | GenderCodes | CodeFlag.Crown) & ~CodeFlag.Face,
            CodeFlag.Manderville  => (FullCodes | RaceCodes | SizeCodes | GearCodes | DyeCodes | GenderCodes | CodeFlag.Crown) & ~CodeFlag.Manderville,
            CodeFlag.Smiles       => (FullCodes | RaceCodes | SizeCodes | GearCodes | DyeCodes | GenderCodes | CodeFlag.Crown) & ~CodeFlag.Smiles,
            _                     => 0,
        };
    // @formatter:on
    ...
    public static string GetDescription(CodeFlag flag)
        => flag switch
        {
            ...
            CodeFlag.AllMale      => "Changes all players to male.",
            CodeFlag.AllFemale    => "Changes all players to female.",
