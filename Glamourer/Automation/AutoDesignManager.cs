...
public sealed class AutoDesignManager : ISavable, IReadOnlyList<AutoDesignSet>, IDisposable, IService
{
    ...
    private void LoadV1(in JsonElement? data)
    {
        ...
        foreach (var obj in array.EnumerateArray())
        {
            ...
            var id = GTActorIdentifierJson.FromJson(_actors, obj.TryReadObject("Identifier"u8, out var i) ? i : null).WithoutIndex();
            ...
            {
                var identifier = GTActorIdentifierJson.FromJson(_actors, idJObj).WithoutIndex();
